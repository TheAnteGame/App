...
## 2026-08-14 — Incident + full restore (parallel-session revert)

- [incident] A parallel session, asked to update the logo, ended up reverting the deployed source tree to a pre-Phase-1.5 state (Phase 2 dashboard, commissioner console, Content studio, hardened job auth — all gone from `main` and deployed that way). Its stale code then queried the dead `open` week state that migration 0005 had removed; it misdiagnosed that as a broken DATABASE and re-added every 0001 enum value to prod (Robert pasted its "repair").
- [restore] **Full tree restored** from this session's clone on top of the incident commits (no force-push, history intact): Phase 1.5 hardening, Phase 2, Content studio, workflows, tests — everything through `a92d35e` is live again.
- [branding] The other session's actual deliverable is PRESERVED: updated + compressed colored logo files (`branding/`, `public/brand/`) verified into the restored tree.
- [db] Its enum re-add is kept as `0007_enum_repair_incident.sql` (renamed to its true chronological slot, header explains the misdiagnosis). Dead enum values (`open`/`locked`/`draft`) exist in prod again — harmless, nothing writes them; optional re-shrink someday. `site_content`/`ticker_items` (0006) and all game data were never touched.
- [cleanup] The incident session's temp `/api/jobs/debug-state` route is gone with the restore.
- [lesson] **One session owns the repo at a time.** Parallel sessions must read ROADMAP/CHANGELOG first (session protocol) — the revert happened because a stale session pushed without syncing context.
